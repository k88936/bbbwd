...
public class ConstructionTest {
    ...
    public void testConnectAnchorCompute() {
        ...
        TestUtils.setBlock(0, 0, 0, Blocks.testBlock);
        InputHandler.seekResult seekResult = Vars.control.inputHandler.seekPlaceForBuild(new Vector2(1, 2*Vars.GRID_SIZE), Blocks.testBlock, 0);
        assert seekResult.connections().get(0).anchorNewBlock().epsilonEquals(-0.5f,-0.25f);
        assert seekResult.connections().get(0).anchorOldBlock().epsilonEquals(0.5f,0.25f);
        assert MathUtils.isEqual(seekResult.connections().get(0).relativeAngle()[0],5.81f,0.1f);
    }
}
